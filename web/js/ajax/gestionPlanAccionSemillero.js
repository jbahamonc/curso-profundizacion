...
$(function () {
    ...
    $("#btn-save-action-plan-semillero").on("click", function() {
        ...
        if ( form.valid() ) {
            ...
            $.ajax({
                url     : '../paginas/procesar/gestionPlanAccionSemillero.jsp?operacion=3',
                type    : 'POST',
                data    : $.param(form),
                success : function ( response ) {
                    ...
                    if (json.status == 200) {
                        ...
                        var ulCapacitaciones = $("#ulCapacitaciones")
                        ulCapacitaciones.empty()
                        if ( json.capacitaciones.length > 0 ) {
                            for (var p of json.capacitaciones) {
                                html += '<li class="item">'+
                                        '<div class="product-img">'+
    ...
    $("#btn-add-project-plan-semillero").on("click", function () {
        var myToast = $.mdtoast('Registro en proceso...', { duration: 1000000, init: true });
        myToast.show()
        var form = $("#form-project-news-plan-semillero")
        if ( form.valid() ) {
            ...
            $.ajax({
                url     : '../paginas/procesar/gestionPlanAccionSemillero.jsp?operacion=4',
                type    : 'POST',
                data    : $.param(form),
                success : function ( response ) {
                    var json = JSON.parse(response)
    ...
    $("#btn-save-act-plan-semillero").on("click", function () {
        ...
        if ( form.valid() ) {
            ...
            $.ajax({
                url     : '../paginas/procesar/gestionPlanAccionSemillero.jsp?operacion=6',
                type    : 'POST',
                data    : $.param(data),
                success : function ( response ) {
                    var json = JSON.parse(response)
    ...
    $("#btn-save-capacity").on("click", function () {
        ...
        if ( form.valid() ) {
            ...
            data.push('semestre', plan.semestre)
            $.ajax({
                url     : '../paginas/procesar/gestionPlanAccionSemillero.jsp?operacion=5',
                type    : 'POST',
                data    : $.param(data),
                success : function ( response ) {
                    var json = JSON.parse(response)
